...
### 输入
- **命令行参数**：通过 `config.yaml` 配置文件指定目标、模块、输出格式等。
- **配置文件**：`config.yaml` 包含目标 URL、扫描模式、超时设置等。

### 输出
- **控制台输出**：实时显示扫描进度和发现的问题。
- **报告文件**：生成 JSON/HTML 格式的报告，保存在 `results/` 目录。
- **日志文件**：详细执行日志保存在 `logs/` 目录。
